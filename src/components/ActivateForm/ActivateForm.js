  ...
  render() {
    return (
      ...
      <form className='Activate' onSubmit={this.props.onhandleActivate} style={{width: "100%", display: "inline-block"}}>
        ...
        <label htmlFor='deposit_amount' style={{fontSize: "12px"}}>Deposit Amount ({this.props.tokenDetails? this.props.tokenDetails.symbol + " tokens" : 'TACO'})</label>
        <input id='deposit_amount' className='SendAmount' type='text' style={{width: "450px", border: "0px"}} defaultValue={this.props.amount}/>
        <button type='submit' className='AddBtn' style={{width: "100%", backgroundColor: "#f52a34", border:"0px", color: "#152639"}}>Activate</button>
      </form>
      </div>
    )
  }
